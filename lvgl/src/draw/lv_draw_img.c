...
//#endif // !1define STB_IMAGE_IMPLEMENTATION
...
lv_img_src_t lv_img_src_get_type(const void * src)
{
    ...
    int result = stbi_info_from_memory(src, 0, &width, &height, &channels);

    if (result != 0 && channels >= 3 && channels <= 4) {
        img_src_type = LV_IMG_SRC_DATA; /* Assume it's a PNG file */
    }
