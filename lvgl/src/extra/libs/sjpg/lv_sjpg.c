...
// #include "/opt/zhoux/workspace/lvgl_demo/lvgl/src/extra/libs/sjpg/nanojpeg.h"
    ...
    if(dsc->src_type == LV_IMG_SRC_VARIABLE) {
        ...
        if(strcmp(lv_fs_get_ext(fn), "sjpg") == 0) {
            ...
            if(rc == JDR_OK) {
                ...
                unsigned char *Image_Data = stbi_load(fn, &width, &height, &channels, STBI_rgb_alpha);
                convert_color_depth(Image_Data, width * height);  
